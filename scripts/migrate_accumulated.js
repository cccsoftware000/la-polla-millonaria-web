...
async function migrateOldAccumulated() {
  ...
  // 2. Buscar la polla activa (Jornada 2)
  ...
  if (!openPolla) {
    // Migrar a pendingCarry si no hay polla abierta
    ...
    console.log('   ⚠️ Se asignará automáticamente al recibir el próximo pago de apuesta');
    return;
  }
  ...
  // Migrar: sumar el old accumulated al prizeAmount de la polla activa
  await db.runTransaction(async (tx) => {
    ...
    // Limpiar el old accumulated y actualizar pendingCarry
    tx.update(settingsRef, {
      currentAccumulated: 0,
      pendingCarry: 0,
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    });
    // Registrar en accumulated_history
